...
//    let index: Int
    let delta: Delta?
//    let logprobs: String? // 根据 JSON 为 null，类型可以调整
//    let finish_reason: String? // 根据 JSON 为 null，类型可以调整
}
 
struct AI302Message: Codable {
//    let id: String
//    let object: String
//    let created: Int
//    let model: String
...
struct AI302MessageData: Codable {
    let data: AI302Message
}

struct Stream302AIMessage: Codable  {
...
enum Stream302AIMessageType {
    case started
    case token(String)
    case webSearch(StreamWebSearch)
    case skip
    
    static func messageType(from jsonModel: String) -> Stream302AIMessageType? {
        
        if jsonModel == "" {
            return .skip
        }else{
            return .token(jsonModel)
            
        }
        
    }
    
    private static func webSearch(from json: StreamMessage) -> Stream302AIMessageType? {
        guard let messageType = json.subtype else { return nil }
